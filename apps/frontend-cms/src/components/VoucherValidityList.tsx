...
export const VoucherValidityList: React.FC<VoucherValidityListProps> = ({
  voucherId,
}) => {
  ...
  const [editingValidity, setEditingValidity] =
    useState<VoucherValidity | null>(null);
  ...
  const handleCreateOrUpdate = async (
    validityPayload: Partial<VoucherValidity>,
  ) => {
    if (editingValidity) {
      await updateVoucherValidity(
        voucherId,
        editingValidity.id,
        validityPayload,
      );
    ...
    <Card className="border-slate-800/50 bg-slate-900/40 backdrop-blur-xl mt-8">
      <CardHeader className="flex flex-row items-center justify-between border-b border-slate-800/50 pb-6">
        <div>
          ...
          <CardDescription>
            Configure when this voucher can be claimed or used.
          </CardDescription>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-sm text-slate-500 mt-2 max-w-sm">
              This voucher has no automated validity schedules. Add schedules to
              automatically activate or deactivate it.
            </p>
          </div>
        ) : (
                    ...
                    <td className="px-6 py-4 text-slate-300">
                      <div className="flex flex-col">
                        ...
                        <span className="text-slate-500 text-xs">
                          to {formatDate(validity.end_date)}
                        </span>
                      </div>
                    </td>
