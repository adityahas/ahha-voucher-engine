...
export const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant = 'primary', size = 'md', isLoading, icon: Icon, iconRight, children, ...props }, ref) => {
    ...
    return (
      ...
      >
        ...
        {!isLoading && Icon && !iconRight && <Icon className="mr-2 h-5 w-5 group-hover:-translate-x-1 transition-transform" />}
        {children}
        {!isLoading && Icon && iconRight && <Icon className="ml-2 h-5 w-5 group-hover:translate-x-1 transition-transform" />}
      </button>
    );
  }
);
