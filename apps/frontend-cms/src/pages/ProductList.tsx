          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 space-y-6">
              <div className="p-6 rounded-2xl bg-red-500/5 border border-red-500/20 max-w-md w-full text-center">
                ...
                <h3 className="text-lg font-bold text-red-200 mb-2">Engine Sync Failed</h3>
                <p className="text-sm text-red-400/80 leading-relaxed font-medium">{error}</p>
              </div>
              <Button variant="outline" onClick={fetchProducts} className="border-red-500/30 text-red-400 hover:bg-red-500/10">
                Retry Sync
              </Button>
            </div>
          ) : products.length === 0 ? (
            <div className="text-center py-24 group">
              ...
              <p className="text-slate-400 text-lg font-medium">No products registered.</p>
              <p className="text-slate-500 text-sm mt-1">Start by adding your first product to the catalog.</p>
            </div>
          ) : (
            <div className="overflow-hidden rounded-xl border border-slate-800/50">
              <Table>
                <TableHeader className="bg-slate-800/30">
                  <TableRow>
                    <TableHead className="font-bold text-slate-300">Product Info</TableHead>
                    <TableHead className="font-bold text-slate-300">SKU</TableHead>
                    <TableHead className="font-bold text-slate-300">Unit Price</TableHead>
                    <TableHead className="font-bold text-slate-300">Stock Availability</TableHead>
                    ...
                    >
                      ...
                      <TableCell>
                        <div className="flex items-center space-x-2">
                           <span className="text-slate-200 font-mono font-bold">{product.stock}</span>
                           <span className="text-xs text-slate-500 font-medium uppercase tracking-tighter">units</span>
                        </div>
                      </TableCell>
                      <TableCell>
                        {product.stock > 10 ? (
