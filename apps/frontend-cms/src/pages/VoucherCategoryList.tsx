...
import { getVoucherCategories, deleteVoucherCategory, VoucherCategory } from '../api/voucher-categories';
import { Loader2, AlertCircle, RefreshCw, LayoutGrid, Plus, Edit2, Trash2 } from 'lucide-react';
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 space-y-6">
              <div className="p-6 rounded-2xl bg-red-500/5 border border-red-500/20 max-w-md w-full text-center">
                ...
                <h3 className="text-lg font-bold text-red-200 mb-2">Failed to load categories</h3>
                <p className="text-sm text-red-400/80 leading-relaxed font-medium">{error}</p>
              </div>
              <Button variant="outline" onClick={fetchCategories} className="border-red-500/30 text-red-400 hover:bg-red-500/10">
                Retry Connection
              </Button>
            </div>
          ) : categories.length === 0 ? (
            <div className="text-center py-24 group">
              ...
              <p className="text-slate-500 text-sm mt-1">Start by creating your first category.</p>
            </div>
          ) : (
            <div className="overflow-hidden rounded-xl border border-slate-800/50">
              <Table>
                <TableHeader className="bg-slate-800/30">
                  <TableRow>
                    <TableHead className="font-bold text-slate-300 w-16 text-center">Image</TableHead>
                    <TableHead className="font-bold text-slate-300">Name</TableHead>
                    <TableHead className="font-bold text-slate-300">Slug</TableHead>
                ...
                <TableBody>
                  {categories.map((category) => (
                    <TableRow
                      ...
                      onClick={() => navigate(`/voucher-categories/${category.slug}`)}
                    >
                      ...
                      <TableCell className="text-right">
                        <div className="flex items-center justify-end gap-2 opacity-50 group-hover:opacity-100 transition-opacity">
                           ...
                           <button
                             onClick={(e) => handleDelete(e, category.slug, category.name)}
