  ...
  if (isFetchingInitial) {
    return (
      <div className="flex flex-col items-center justify-center py-20 animate-in fade-in duration-500">
        ...
        <p className="mt-4 text-slate-400 font-medium">Synchronizing campaign data...</p>
      </div>
    );
  }
      ...
      <div className="flex flex-col items-center justify-center py-20 animate-in zoom-in-95 duration-500">
        <div className="p-8 rounded-2xl bg-green-500/10 border border-green-500/30 flex flex-col items-center space-y-4 max-w-md text-center backdrop-blur-md shadow-[0_0_40px_rgba(34,197,94,0.15)]">
          ...
          <p className="text-green-200/80 font-medium">
            Campaign parameters have been updated across the engine. Redirecting...
          </p>
        </div>
      </div>
    );
    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-700">
      <div className="flex items-center space-x-4">
        ...
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1 font-medium font-mono text-primary-400/80 tracking-tighter uppercase">{code}</p>
        </div>
      </div>
          ...
          <div className="lg:col-span-2 space-y-8">
            <Card className="border-slate-700/50 relative overflow-hidden bg-slate-900/40 backdrop-blur-xl">
              ...
              <CardContent className="space-y-6 pt-2">
                ...
                <div className="space-y-2">
                  <label htmlFor="description" className="text-xs font-bold text-slate-400 uppercase tracking-widest ml-1">Narrative</label>
                  <textarea
                    id="description"
                    name="description"
                ...
                <div className="space-y-2">
                  <label htmlFor="image" className="text-xs font-bold text-slate-400 uppercase tracking-widest ml-1">Asset URL</label>
                  <div className="relative">
                     <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                 ...
                 <div className="flex flex-wrap gap-2">
                    {availableCategories.map(cat => (
                      ...
                      >
                         {cat.name}
                         {formData.categories.includes(cat.slug) ? (
                            <X size={12} className="ml-2 text-primary-400 opacity-50" />
                         ) : (
                            <Plus size={12} className="ml-2 text-slate-600 opacity-50" />
                         )}
                      </button>
                    ))}
                 </div>
