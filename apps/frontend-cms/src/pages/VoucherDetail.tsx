        ...
        <div className="glass-dark border border-red-500/20 rounded-3xl p-8 text-center space-y-6 shadow-2xl shadow-red-500/5">
          ...
          <div>
            ...
            <p className="text-slate-400 mt-2 font-medium">
              {error ||
                'The voucher code you are looking for does not exist or has been removed.'}
            </p>
          </div>
          <Button
            variant="outline"
        ...
        <div className="lg:col-span-2 space-y-8">
          <Card className="border-slate-800/50 bg-slate-900/40 backdrop-blur-xl overflow-hidden group">
            <CardHeader className="border-b border-slate-800/50 pb-6 relative">
              ...
              <CardDescription>
                Primary configuration and campaign details.
              </CardDescription>
            </CardHeader>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2 p-3 rounded-xl bg-slate-800/30 border border-slate-700/50">
                      {voucher.categories?.map((cat) => (
                        <span
                          key={cat.slug}
                          className="text-xs font-bold px-3 py-1 rounded-lg bg-primary-500/10 text-primary-400 border border-primary-500/20"
                        >
                          {cat.name}
                        </span>
                      )) || (
          ...
          <Card className="border-slate-800/50 bg-slate-900/40 backdrop-blur-xl">
            <CardHeader>
              ...
              <CardDescription>
                System events and lifecycle tracking.
              </CardDescription>
            </CardHeader>
            ...
            <CardContent className="space-y-6">
              <div className="flex items-center justify-between p-4 rounded-2xl bg-slate-800/20 border border-slate-800/50">
                <div className="flex items-center space-x-3">
                  <ShieldCheck
                    size={20}
                    className={
                      voucher.quota > 0 ? 'text-green-500' : 'text-amber-500'
                    }
                  />
                ...
                <span
                  className={`text-xs font-bold px-3 py-1 rounded-full ${voucher.quota > 0 ? 'bg-green-500/10 text-green-400 border border-green-500/20' : 'bg-amber-500/10 text-amber-500 border border-amber-500/20'}`}
                >
              ...
              <div className="p-6 rounded-2xl bg-gradient-to-br from-primary-500/10 to-purple-500/10 border border-white/5 space-y-4">
                ...
                <p className="text-xs text-slate-400 leading-relaxed font-medium">
                  This voucher is isolated to your current workspace. All
                  issuance and claims are tracked under the multi-tenant
                  architecture.
                </p>
              </div>
            </CardContent>
          ...
          <Card className="border-slate-800/50 bg-slate-900/40 backdrop-blur-xl">
            <CardHeader className="pb-2">
              ...
              <CardDescription className="text-xs">
                Users explicitly targeted by this campaign.
              </CardDescription>
            </CardHeader>
            <CardContent>
                ...
                {voucher.target_users && voucher.target_users.length > 0 ? (
                  <div className="space-y-2 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
                    {voucher.target_users.map((target) => {
                      const userDetail = allUsers.find(
                        (u) => u.id === target.core_user_id,
                      );
                      return (
                        <div
                          key={target.id}
                          className="p-3 rounded-xl bg-slate-800/30 border border-slate-700/50 flex items-center gap-3 group hover:border-orange-500/30 transition-colors"
                        >
                  ...
                  <div className="text-center py-6 border border-dashed border-slate-800 rounded-2xl">
                    <p className="text-xs text-slate-600 italic">
                      No specific users targeted. This campaign is public
                      (global).
                    </p>
                  </div>
                )}
