...
describe('FeedbackOverlay', () => {
  ...
  it('renders processing state with spinner', () => {
    ...
    expect(document.querySelector('.lucide-loader-circle') || document.querySelector('.lucide-loader-2')).toBeInTheDocument();
  });

  it('renders success state with message', () => {
    const onClose = vi.fn();
    render(<FeedbackOverlay status="success" message="Well done!" onClose={onClose} />);
  ...
  it('renders error state with message', () => {
    const onClose = vi.fn();
    render(<FeedbackOverlay status="error" message="Something exploded" onClose={onClose} />);
  ...
  it('applies Vibe Coding aesthetic classes', () => {
    ...
    const container = screen.getByText('Purchase Successful!').closest('.bg-white\\/10');
